...
function habits() { const today = actionItems(dateKey()); const items = state.habits.map((habit) => ({ ...habit, done: !!today.find((item) => item.kind === "habit" && item.id === habit.id)?.done })); return `<section class="page-title"><p>CONFIGURACIÓN PERSONAL</p><h1>Hábitos</h1><button class="add-button" data-act="new-habit">${svg.plus} Nuevo hábito</button></section><section class="manage-list">${items.length ? items.map((h) => row(h, "habit")).join("") : `<p class="empty-state">Crea tu primer hábito.</p>`}</section>`; }
